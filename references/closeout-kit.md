...
| --- | --- | --- |
| Claude Fable peer feedback | The goal or plan is difficult, ambiguous, high-risk, or benefits from critique plus feature and science ideation by another model. | `yes` plus a 1-10 round count selects the Anthropic sequence; one bounded native approval covers every configured round inside the disclosed goal-path envelope. |
...
| GPT Pro review | The plan or implementation is difficult, ambiguous, scientific, high-risk, or needs a high-context independent gate. | `yes` selects a bounded DevSpace-style MCP workspace, fallback delivery, one exact-packet submission, full response custody, and typed reconciliation without another plugin. |
...
Record the answers in the exact `## Closeout options` table and the Fable and GPT Pro selectors in frontmatter. `ask` means the skill is waiting at the planning checkpoint. A Fable `yes` selects the lane; one native approval of its bounded goal authorization covers later changed manifests inside that envelope. Never ask for a typed approval sentence or another per-round confirmation. A GPT Pro `yes` pre-approves only the exact generated request and packet for ChatGPT Pro; expanded data or destination requires confirmation.
...
When selected, run the external-review approval preflight during planning, then run `run_fable_feedback.py --authorize-goal` once through the native owner approval surface. The recorded envelope covers the goal directory, explicit additional files, configured planning and rescue counts, destination, model, efforts, and byte ceiling. Every call still emits an exact manifest and durable response evidence, but covered later rounds run without another owner prompt. Claude receives only embedded covered files and has no local repository tools. Reconcile each round before the next one.
...
When selected, use Goal Ledger's own [pro-review.md](pro-review.md) and `scripts/run_pro_review.py`; never invoke or depend on a separate `$pro` skill. Prefer the bundled MCP workspace when [review-bridge.md](review-bridge.md) preflight passes: Pro gets `open_workspace`, `list_files`, `read`, `search`, and `write_review`, while shell, live repository, edits, and arbitrary writes remain unavailable. Otherwise route the exact artifacts through native Chat, Safari/Chrome, or owner handoff. Preserve and reconcile the complete response.
