...
| --- | --- | --- |
| Claude Fable peer feedback | The goal or plan is difficult, ambiguous, high-risk, or benefits from critique plus feature and science ideation by another model. | `yes` plus a 1-10 round count authorizes preparing that Anthropic review sequence; every exact hashed manifest still requires owner-facing native Codex approval, with one evidence artifact per reconciled round. |
...
| GPT Pro review | The plan or implementation is difficult, ambiguous, scientific, high-risk, or needs a high-context independent gate. | `yes` selects native prompt-plus-ZIP preparation, the bundled restricted MCP App when configured, Safari/Chrome or owner fallback, one exact-packet submission, full response custody, typed reconciliation, and configured stage/gate behavior without a separate skill or plugin dependency. |
...
Ask immediately after the bounded discovery needed to make those recommendations and follow [planning-controls.md](planning-controls.md). When `request_user_input` is available, use its native click controls for every boolean and selector; never replace it with a prose `yes/no` reply request. Use a stepped model-family/effort selector because the current tool has no literal range slider or multi-select checkbox group. When native input is unavailable, use the already-connected bundled MCP App for real checkboxes and selectors; use one concise Markdown checklist only when that app is unavailable. Do not make app provisioning a new planning blocker. Do not begin implementation, delegation, long-running commands, or unattended execution before the answers arrive, and never move this checkpoint to the final closeout response.

Record the answers in the exact `## Closeout options` table and the Fable and GPT Pro selectors in frontmatter. `ask` means the skill is waiting at the planning checkpoint; it is not a default recommendation or permission to proceed. A Fable `yes` authorizes preparing that lane within the recorded scope; exact transmission approval remains a separate native checkbox after manifest disclosure. Never ask for a typed approval sentence. A GPT Pro `yes` pre-approves only the exact generated request and packet for ChatGPT Pro; expanded data or destination requires confirmation. Do not close a goal while any choice remains `ask`.
...
When selected, run the external-review approval preflight during planning, then prepare the next configured round before implementation. The runner first emits an exact allow-list manifest without contacting Claude. Submit the matching digest-bound command through Codex's owner-facing native external-transmission checkbox rather than asking for a prose reply. Claude receives only embedded approved files, has no local repository tools, and returns critique, feature opportunities, and science/research hypotheses. Reconcile findings and proposal decisions, then validate the updated ledger before preparing the next round. Run the approved direct `claude -p` command outside the sandbox; a sandboxed logged-out result is not a valid authentication diagnosis. Use tmux only as an authenticated fallback for the same approved command, never as an approval bypass. Use High effort by default, XHigh only for difficult or high-risk planning, and never select Max automatically. Results are advisory; adjacent and future proposals do not expand the active goal without user authorization. See [fable-peer.md](fable-peer.md).
...
When selected, use Goal Ledger's own [pro-review.md](pro-review.md) and `scripts/run_pro_review.py`; never invoke or depend on a separate `$pro` skill. Prepare one GPT-5.6-shaped request plus a deterministic, explicitly scoped ZIP. Prefer the bundled restricted MCP App when [review-bridge.md](review-bridge.md) preflight passes: it exposes only manifest-listed ZIP members and writes only immutable response custody through OpenAI Secure MCP Tunnel. Otherwise route the exact artifacts through Safari/Chrome Computer Use or owner handoff. Do not use the ChatGPT desktop/classic app as a Computer Use route. Submit the exact packet once without a second prose approval gate. Preserve the complete raw response, verify every finding locally, and record typed reconciliation. Required plan review gates Build and required implementation review gates completion. Every round is immutable and recovery resumes from `state.json` without duplicate submission.
