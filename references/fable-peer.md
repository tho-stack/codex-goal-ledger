...
The planning checkbox plus `fable_review_rounds` count select 1-10 external, read-only Claude Fable rounds for the current goal. Each round critiques the plan and proposes grounded feature and science/research opportunities. Do not ask for another conversational consent sentence after `yes` is recorded. At action time, automatically prepare an exact hashed file manifest and submit it through Codex's native external-transmission approval layer. Default to one round. Recommend two when a follow-up critique after reconciliation adds material value; use more than three only when the user explicitly wants the added usage.
...
The runner captures Claude stdout and stderr into `evidence/fable-transport/planning-round-N/attempt-N/`, records the PID and invocation digest, flushes and atomically finalizes raw output before parsing, and then writes the human feedback artifact. If an outer Codex wrapper loses stdout, rerun the identical manifest-bound runner command. A completed matching transport is reused without another Claude call; a live matching PID causes a duplicate-submission refusal. Inspect these durable files before diagnosing an empty wrapper result or retrying manually.

Run the bound command directly with Codex `require_escalated` and a justification that names the destination, manifest digest, file list, and total bytes. This is the action-time native approval; do not turn it into a prose reply gate in `progress.md`. The runner uses `claude --print` with model `claude-fable-5`, safe mode, `dontAsk`, WebSearch/WebFetch only, structured JSON output, and no session persistence. Default to `high` effort. Use `xhigh` only when the plan is difficult, ambiguous, high-risk, or an initial High pass leaves material uncertainty. Never select `max` automatically. It removes `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, and `NODE_OPTIONS` from the child environment so the logged-in CLI path is used instead of a stray API credential. It never falls back to another model.
