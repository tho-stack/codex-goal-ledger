...
1. Claude Fable peer feedback — recommend `yes` when the plan is difficult, ambiguous, high-risk, or benefits from feature and science ideation by a second model before implementation; ask for 1-10 rounds (default 1) and explain that each selected round will use a fresh exact hashed file manifest submitted automatically to Codex's native approval layer;
