...
3. native GPT Pro review — recommend `yes` for difficult, ambiguous, scientific, or high-risk work; prefer one required plan round through the bundled restricted `mcp-app` when its preflight passes and otherwise use platform-aware `auto-ui`, while offering explicit Safari, Chrome, owner-handoff, stage, gate, and 1-3-round selectors;
...
When GPT Pro review is `yes`, follow [pro-review.md](pro-review.md) without invoking a separate `$pro` skill. Prepare an immutable prompt plus scoped ZIP after the selected stage is stable. With `mcp-app`, follow [review-bridge.md](review-bridge.md): bind the bundled server to that exact packet, connect through OpenAI Secure MCP Tunnel, and let Pro read only manifest-listed ZIP members before storing the complete response. In `auto-ui`, probe the generated platform order with Computer Use and record every result. Submit once through the selected MCP App, Safari, or Chrome surface; do not probe the ChatGPT desktop/classic app. The planning `yes` authorizes one submission of the exact generated packet, so do not add a prose `Press Send` gate; obey native confirmations that actually appear. If no assisted surface is ready, present the generated checksum-bound owner handoff. Preserve the entire raw response and record typed local reconciliation. A required plan lane gates Build; a required implementation lane gates completion. Recover from the round state instead of resubmitting after interruption.
