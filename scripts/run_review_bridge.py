...
class ReviewScope:
    ...
    def summary(self) -> dict[str, Any]:
        manifest, members = self.verified_packet()
        ...
        return {
            ...
            "request": members["START-HERE.md"].decode("utf-8"),
            "members": [
                {
                    "path": item["path"],
                    "bytes": item["bytes"],
...
PLANNING_SCHEMA: dict[str, Any] = {
    ...
    "pro_deliveries": ["mcp-app", "auto-ui", "safari-assisted", "chrome-assisted", "owner-handoff"],
}
...
def build_server(
    ...
    *,
    ...
    server = FastMCP(
        "Codex Goal Ledger",
        instructions=(
            "Open the Goal Ledger console first. In review mode, use only the bound immutable "
            "packet. Begin only in a visibly selected GPT Pro mode, read every listed packet "
            "member, answer the embedded request, and submit the complete response through "
            "submit_pro_review_response. Never infer live repository access."
        ),
        host=host,
        port=port,
        stateless_http=True,
    ...
    def read_review_file(member_path: str) -> Any:
        ...
        if not submission_path.is_file():
            raise LedgerError("begin the bound Pro review before reading packet members")
        submission = _json_object(submission_path)
        if submission.get("transport") != "mcp-app":
            raise LedgerError("this review round was submitted through another transport")
    ...
    def begin_pro_review(model_visible: str, thread_reference: str) -> Any:
        ...
        if not any(
            ...
            record_transport_attempt(
                ...
                result="ready",
                detail=(
                    "The restricted Goal Ledger MCP App reached the manifest-bound local server; "
                    "the owner confirmed a visible Pro mode in the app control."
                ),
            )
        record_submission(
            scope.goal_dir,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "check":
            ...
            print("Security boundary: immutable ZIP reads; response-only custody write; no shell or live repo tools.")
            if scope is not None:
